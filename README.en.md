...
Bowling Player is a Fabric and Forge mod for Minecraft 1.20.1. The base `bowlingplayer` mod lets players switch between normal, bowling pin, and bowling ball forms. This repository also contains the `bowlingplayergame` add-on for running a cops and robbers game.

## Installation

- Fabric: Install Fabric Loader and Fabric API. Put the base JAR from `fabric/build/libs/` in your `mods` folder. For the game add-on, also put the JAR from `game-fabric/build/libs/` there.
- Forge: Install Forge. Put the base JAR from `forge/build/libs/` in your `mods` folder. For the game add-on, also put the JAR from `game-forge/build/libs/` there.
- Use the base mod and add-on built for the same loader. Select the JAR without a `-sources` or `-javadoc` suffix.
...
The following operator guide is also available in [game-common/README.en.md](game-common/README.en.md).
